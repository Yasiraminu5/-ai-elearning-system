...
const createQuiz = async (req, res) => {
  try {
    ...
    const quiz = await Quiz.create({ courseId, title, questions, passMark: passMark || 50, difficultyLevel: difficultyLevel || 'medium', timeLimit: timeLimit || 30 });
...
const getQuizzes = async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      quizzes = await Quiz.find({ courseId: { $in: courseIds } }).populate('courseId', 'title category');
    }
...
const getQuiz = async (req, res) => {
  try {
    const quiz = await Quiz.findById(req.params.id).populate('courseId', 'title category');
    if (!quiz) return res.status(404).json({ success: false, message: 'Quiz not found' });
    if (req.user.role === 'student') {
      const safeQuiz = {
        _id: quiz._id, title: quiz.title, courseId: quiz.courseId,
        passMark: quiz.passMark, difficultyLevel: quiz.difficultyLevel,
...
const submitQuiz = async (req, res) => {
  try {
    ...
    // Automatically refresh recommendations after quiz submission
    generateRecommendations(req.user._id).catch(err =>
      console.error('Failed to refresh recommendations:', err.message)
    );

    res.status(201).json({
      success: true,
      result: { score, passed, correct, total: quiz.questions.length, passMark: quiz.passMark, answers: gradedAnswers },
    });
  } catch (err) {
